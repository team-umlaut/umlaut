...
(function ($) {
  ...
  //Callback default to jQuery's no-op function
  var noop = $.noop;
  ...
  SectionTarget.prototype.ensure_placement_destination = function () {
    // Short circuit if we don't know where to put our service responses
    if (this.selector === undefined) {
      return null;
    }
  ...
  function HtmlUpdater(umlaut_base, context_object) {
    ...
    this.update = function () {
      ...
      var myself, dataType;
      myself = this;
      dataType = this.is_remote_url(this.umlaut_uri) ? "jsonp" : "json";
      $.ajax({
        ...
        success: function (umlaut_response) {
          var i, section_target, umlaut_html_section, count, existing_element, new_element, should_continue, refresh_seconds;
          for (i = 0; i < myself.section_targets.length; i++) {
            section_target = myself.section_targets[i];
            umlaut_html_section = myself.find_umlaut_response_section(umlaut_response, section_target.umlaut_section_id);
            // If we have a response, process it
            if (umlaut_html_section !== undefined) {
              count = null;
              if (umlaut_html_section.response_count !== undefined) {
                count = parseInt(umlaut_html_section.response_count.value, 10);
              }
              existing_element = section_target.ensure_placement_destination();
              new_element = $('<div class="umlaut" style="display:none" class="' + section_target.umlaut_section_id + '"></div>');
              new_element.html(umlaut_html_section.html_content);
              should_continue = section_target.before_update(new_element, count, section_target);
